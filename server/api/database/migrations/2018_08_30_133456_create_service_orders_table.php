...
class CreateServiceOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('service_orders', function (Blueprint $table) {
            ...
            $table->tinyInteger('is_out')->length(1)->default(2)->comment('是否上门服务: 否 0，是 1, 待定 2');
            $table->datetime('plan_out_at')->nullable()->comment('预计上门时间');
            $table->datetime('plan_finish_at')->nullable()->comment('预计完成时间');
            $table->tinyInteger('is_charge')->length(1)->default(2)->comment('是否服务收费: 否 0，是 1, 待定 2');
            $table->tinyInteger('is_quote')->length(1)->default(2)->comment('是否报价: 否 0，是 1, 待定 2');
            $table->string('attach_ids',50)->nullable()->comment('附件ids');
            $table->string('remark')->nullable()->comment('备注');
            $table->tinyInteger('settle_status')->length(1)->default(0)->comment('结算: 未结算 0, 结算中 1, 已结算 2, 免费 3');
            $table->tinyInteger('status')->length(1)->default(0)->comment('单据状态: 制单中 0, 已受理 1,待派单 2,处理中 3,已取消 4,已关闭 5,无法处理 6');
