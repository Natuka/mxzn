...
class CreateCustomersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('customers', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name',255)->default('')->comment('公司名称');
            $table->string('name_short',255)->comment('公司简称');
            ...
            $table->timestamps('follow_up_nexttime')->comment('下次跟进时间');
            $table->timestamps('contact_lasttime')->comment('最近联系时间');
            $table->integer('province_id')->comment('所在省');
            $table->integer('city_id')->comment('市');
            $table->string('district_county_id')->comment('区县');
            $table->string('address',255)->comment('详细地址');
            $table->string('tel',30)->comment('电话');
            $table->string('fax',30)->comment('传真');
            $table->string('zip_code',10)->comment('邮编');
            $table->integer('salesman_id')->comment('业务员');
            $table->string('ent_code',20)->comment('企业统一信用码');
            $table->string('banak',50)->comment('开户行');
            $table->string('account_number',255)->comment('银行帐号');
            ...
            $table->timestamps('created_time')->comment('创建时间');
            $table->string('updated_by')->comment('修改人');
            $table->timestamps('updated_time')->comment('修改时间');
            $table->rememberToken();
            $table->softDeletes();
            $table->timestamps();
        });
