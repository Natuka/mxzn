...
export default {
  ...
  data () {
    return {
      ...
      educationList: confirmConst.EDUCATION_LIST,
      select: {
        job: [],
        post: [],
        education: []
  ...
  methods: {
    onSubmit (e) {
      this.$refs.addForm.validate(async (valid) => {
        if (valid) {
          try {
            let data = await updateConfirm(this.data, this.data.id)
