...
import {orderServiceSwitch} from '@/api/order_flow/action'
...
export default {
  ...
  methods: {
    async afterOpen () {
      ...
      let organizations = await selectOrganization({id: data.dispatch_org_id})
    ...
    onSubmit (e) {
      ...
      refs['dispatchForm'].validate(async (valid) => {
        if (valid) {
          await orderServiceSwitch(this.data.id, 'repair', this.data)
          this.withRefresh(e)
        } else {
          this.closeLoading()
        }
    ...
    async organizationChange (id) {
      ...
      let engineers = await selectEngineer(this.params.org_id)
      this.init.engineer = engineers.data
    },
