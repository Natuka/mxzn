...
export default {
  ...
  data () {
    return {
      ...
      educationList: disposeConst.EDUCATION_LIST,
      select: {
        job: [],
        post: [],
        education: []
  ...
  methods: {
    onSubmit (e) {
      this.$refs.addForm.validate(async (valid) => {
        if (valid) {
          try {
            let data = await updateDispose(this.data, this.data.id)
