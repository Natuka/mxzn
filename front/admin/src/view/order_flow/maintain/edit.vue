...
export default {
  ...
  data () {
    return {
      ...
      educationList: maintainConst.EDUCATION_LIST,
      select: {
        job: [],
        post: [],
        education: []
  ...
  methods: {
    onSubmit (e) {
      this.$refs.addForm.validate(async (valid) => {
        if (valid) {
          try {
            let data = await updateMaintain(this.data, this.data.id)
