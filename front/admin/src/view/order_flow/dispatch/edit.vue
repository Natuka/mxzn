...
export default {
  ...
  data () {
    return {
      ...
      educationList: dispatchConst.EDUCATION_LIST,
      select: {
        job: [],
        post: [],
        education: []
  ...
  methods: {
    onSubmit (e) {
      this.$refs.addForm.validate(async (valid) => {
        if (valid) {
          try {
            let data = await updateDispatch(this.data, this.data.id)
